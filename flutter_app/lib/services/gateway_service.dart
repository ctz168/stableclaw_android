...
class GatewayService {
  ...
  /// Patch /root/.stableclaw/stableclaw.json to clear denyCommands and set
  ...
  Future<void> _writeNodeAllowConfig() async {
    ...
    // Use a Node.js one-liner to safely merge into existing stableclaw.json
    // without clobbering other settings (API keys, onboarding config, etc.)
    final allowJson = jsonEncode(allowCommands);
    final script = '''
const fs = require("fs");
const p = "/root/.stableclaw/stableclaw.json";
let c = {};
...
''';
    ...
    if (!prootOk) {
      try {
        final filesDir = await NativeBridge.getFilesDir();
        final configFile = File('$filesDir/rootfs/ubuntu/root/.stableclaw/stableclaw.json');
  ...
  /// Repair stableclaw.json on disk — fixes corrupted model entries and ensures
  ...
  /// Read the actual gateway auth token from stableclaw.json config file (#74, #82).
  /// This is the source of truth — more reliable than regex-scraping stdout.
  Future<String?> _readTokenFromConfig() async {
    try {
      final raw = await NativeBridge.readRootfsFile('root/.stableclaw/stableclaw.json');
  ...
  Future<void> _checkHealth() async {
    try {
      ...
      if (response.statusCode < 500 && _state.status != GatewayStatus.running) {
        // Read the actual token from stableclaw.json — source of truth (#74, #82).
