...
class NodeService {
  ...
  /// 3. stableclaw.json config file (source of truth — fallback when URL is stale or missing)
  Future<String?> _readGatewayToken() async {
    ...
    // 3. Read directly from stableclaw.json — the source of truth (#94).
    ...
    try {
      ...
      if (raw != null) {
        ...
        if (token is String && token.isNotEmpty) {
          _log('[NODE] Gateway token read from stableclaw.json config');
          return token;
        }
      }
    } catch (e) {
      _log('[NODE] Could not read token from stableclaw.json: $e');
    }
    ...
    try {
      ...
      if (code != null) {
        ...
        if (isLocal) {
          ...
          try {
            await NativeBridge.runInProot('stableclaw nodes approve $code');
