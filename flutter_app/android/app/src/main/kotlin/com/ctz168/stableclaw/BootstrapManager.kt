...
class BootstrapManager(
    ...
    fun setupDirectories() {
        listOf(rootfsDir, tmpDir, homeDir, configDir, "$homeDir/.stableclaw", libDir).forEach {
            File(it).mkdirs()
        }
    ...
    fun isBootstrapComplete(): Boolean {
        ...
        val bypass = File("$rootfsDir/root/.stableclaw/bionic-bypass.js")
        val node = File("$rootfsDir/usr/local/bin/node")
        val stableclaw = File("$rootfsDir/usr/local/lib/node_modules/stableclaw/package.json")
        return rootfs.exists() && binBash.exists() && bypass.exists()
            && node.exists() && stableclaw.exists()
    }
    ...
    fun getBootstrapStatus(): Map<String, Any> {
        ...
        val stableclawExists = File("$rootfsDir/usr/local/lib/node_modules/stableclaw/package.json").exists()
        val bypassExists = File("$rootfsDir/root/.stableclaw/bionic-bypass.js").exists()

        return mapOf(
            "rootfsExists" to rootfsExists,
            "binBashExists" to binBashExists,
            "nodeInstalled" to nodeExists,
            "stableclawInstalled" to stableclawExists,
            "bypassInstalled" to bypassExists,
            "rootfsPath" to rootfsDir,
            "complete" to (rootfsExists && binBashExists && bypassExists
                && nodeExists && stableclawExists)
        )
    }
    ...
    private fun configureRootfs() {
        ...
        aptConfDir.mkdirs()
        File(aptConfDir, "01-stableclaw-proot").writeText(
        ...
        dpkgConfDir.mkdirs()
        File(dpkgConfDir, "01-stableclaw-proot").writeText(
            "force-unsafe-io\n" +
            "no-debsig\n" +
            "force-overwrite\n" +
            "force-depends\n" +
        ...
        listOf(
            ...
            // StableClaw runtime directories (can't mkdir at runtime)
            "$rootfsDir/root/.stableclaw",
            "$rootfsDir/root/.stableclaw/data",
            "$rootfsDir/root/.stableclaw/memory",
            "$rootfsDir/root/.stableclaw/skills",
            "$rootfsDir/root/.stableclaw/config",
            "$rootfsDir/root/.stableclaw/extensions",
            "$rootfsDir/root/.stableclaw/logs",
            "$rootfsDir/root/.config/stableclaw",
            "$rootfsDir/root/.local/share",
            "$rootfsDir/root/.cache",
            "$rootfsDir/root/.cache/stableclaw",
    ...
    fun createBinWrappers(packageName: String) {
        ...
        // "bin": "cli.js"  OR  "bin": {"stableclaw": "bin/stableclaw.js", ...}
    ...
    fun installBionicBypass() {
        val bypassDir = File("$rootfsDir/root/.stableclaw")
        if (!bypassDir.exists()) {
            bypassDir.mkdirs()
        }
        ...
        val cwdFixContent = """
// StableClaw CWD Fix - Auto-generated
...
""".trimIndent()
        ...
        //    Usage: node /root/.stableclaw/node-wrapper.js <script> [args...]
        val wrapperContent = """
// StableClaw Node Wrapper - Auto-generated
...
require('/root/.stableclaw/proot-compat.js');
...
// StableClaw Proot Compatibility Layer - Auto-generated
...
""".trimIndent()
        ...
        // 4. Bionic bypass — comprehensive runtime patcher for stableclaw.
        //    Loaded via NODE_OPTIONS="--require /root/.stableclaw/bionic-bypass.js"
        val bypassContent = """
// StableClaw Bionic Bypass - Auto-generated
...
""".trimIndent()
        ...
        val exportLine = "export NODE_OPTIONS=\"--require /root/.stableclaw/bionic-bypass.js\""

        val existing = if (bashrc.exists()) bashrc.readText() else ""
        if (!existing.contains("bionic-bypass")) {
            bashrc.appendText("\n# StableClaw Bionic Bypass\n$exportLine\n")
        }

        // Pre-seed stableclaw.json with gateway.mode=local so the gateway
        // doesn't reject startup with "set gateway.mode=local" (#93, #90).
        val configFile = File(bypassDir, "stableclaw.json")
...
""".trimIndent())
    ...
    /** Read a file from inside the rootfs (e.g. /root/.stableclaw/stableclaw.json). */
    ...
    private fun checkStableClawInProot(): Boolean {
        return try {
            val pm = ProcessManager(filesDir, nativeLibDir)
            val output = pm.runInProotSync("command -v stableclaw")
